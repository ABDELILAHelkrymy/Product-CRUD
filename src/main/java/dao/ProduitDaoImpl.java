...
public class ProduitDaoImpl implements IProduitDao{
	...
	public Produit save(Produit p) {
		...
		try {
			PreparedStatement ps = connection.prepareStatement("INSERT INTO PRODUITS (DESIGNIATION, PRIX, QUANTITE) VALUES (?,?,?)");
	...
	public List<Produit> produitParMC(String mc) {
		...
		Connection connection = SingletonConnection.getConnection();
		try {
			PreparedStatement ps = connection.prepareStatement("SELECT * FROM PRODUITS WHERE DESIGNIATION LIKE ?");
			ps.setString(1, mc);
	...
	public Produit update(Produit p) {
		Connection connection = SingletonConnection.getConnection();
		try {
			PreparedStatement ps = connection.prepareStatement("UPDATE PRODUITS SET DESIGNIATION=?, PRIX=?, QUANTITE=? WHERE ID=?");
			ps.setString(1, p.getDesigniation());
			ps.setDouble(2, p.getPrix());
			ps.setInt(3, p.getQuantite());
